...
class DecisionCrew:
    ...
    # ── execução ────────────────────────────────────────────────────────────────
    def run(self, snapshot: dict, weather: dict, data_date: str) -> FarmDecision:
        ...
        soil_task = Task(
            description=f"Interprete o snapshot de sensores do dia {data_date} (sem decidir). "
        ...
        irrigation_task = Task(
            description="Com base nos resumos de clima e solo, decida se a lavoura deve ser "
                        "irrigada agora e quantos mm. Seja conservador: não irrigue se há "
        ...
        nitrogen_task = Task(
            description="Com base no resumo de solo/nutrição, decida se deve aplicar nitrogênio "
                        "agora e quantos kg/ha. Evite aplicar perto da maturação (DVS alto) ou "
